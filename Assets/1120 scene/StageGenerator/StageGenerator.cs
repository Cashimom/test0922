...
public class StageGenerator : MonoBehaviour
{
    ...
    virtual public List<SavingObject> Generate()
    {
        ...
        GenerateBuilt(vec.vec3(-1, (float)(size.y - 1) / 2f, (float)(size.z - 1) / 2f), Built.N2)
            .transform.localScale = vec.vec3(blockSize.x / transform.localScale.x, 1.1f, 1.1f);
        GenerateBuilt(vec.vec3(size.x, (float)(size.y - 1) / 2f, (float)(size.z - 1) / 2f), Built.N2)
            .transform.localScale = vec.vec3(blockSize.x / transform.localScale.x, 1.1f, 1.1f);

        GenerateBuilt(vec.vec3((float)(size.x - 1) / 2f, (float)(size.y - 1) / 2f, -1), Built.N2)
            .transform.localScale = vec.vec3(1.1f, 1.1f, blockSize.z / transform.localScale.z);
        GenerateBuilt(vec.vec3((float)(size.x - 1) / 2f, (float)(size.y - 1) / 2f, size.z), Built.N2)
            .transform.localScale = vec.vec3(1.1f, 1.1f, blockSize.z / transform.localScale.z);

        GenerateBuilt(vec.vec3((float)(size.x - 1) / 2f, -1, (float)(size.z - 1) / 2f), Built.N2)
            .transform.localScale = vec.vec3(1.1f, blockSize.y / transform.localScale.y, 1.1f);
        GenerateBuilt(vec.vec3((float)(size.x - 1) / 2f, size.y, (float)(size.z - 1) / 2f), Built.N2)
            .transform.localScale = vec.vec3(1.1f, blockSize.y / transform.localScale.y, 1.1f);
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                for (int z = 0; z < size.z; z++)
                {
                    ...
                    {
                        GenerateBuilt(vec.vec3(x, y, z), (Built)map[x, y, z]);
                    }
    ...
    Vector3Int RandomPosition(int side)
    {
        return new Vector3Int(Random.Range(side, size.x - side), Random.Range(side, size.y - side - 1), Random.Range(side, size.z - side));
    }
